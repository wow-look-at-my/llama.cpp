...
extern "C" {
    ...
    struct llama_model_params {
        ...
        bool use_mmap;        // allow mmap; only used when weights can be used directly from the mapping
                              // (e.g. CPU or unified memory) - weights bound to other devices are streamed
                              // through pinned staging buffers instead
    ...
    // Gemma 4 MTP: load a gemma4_assistant GGUF onto a gemma4 target (call after
    // llama_model_load_from_file, before llama_init_from_model). Returns 0 on success.
