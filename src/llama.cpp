...
//
// Gemma 4 MTP: load the assistant (draft) model onto a target model
//
...
int llama_model_load_mtp_from_file(struct llama_model * model, const char * path_mtp, struct llama_model_params params) {
    ...
    // Rename all MTP assistant tensors with an "mtp." prefix so they can be uniquely targeted by
    // -ot rules without colliding with the main model's tensors. Tensors already prefixed with
    // "mtp." (pre_projection, post_projection, centroids, token_ordering) are left unchanged.
